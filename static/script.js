...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (reviewForm) {
    reviewForm.addEventListener("submit", async (e) => {
      ...
      if (!star)
        return Swal.fire("Atenção", "Selecione uma estrela!", "warning");
  ...
  window.checkAndStart = async function (categoria) {
    ...
    if (history.length > 0) {
      try {
        const res = await fetch("/my_tickets", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ uuids: history }),
        });
        const tickets = await res.json();
        const openTicket = tickets.find(
          (t) => t.category === categoria && t.status === "Aberto"
        );
        if (openTicket) {
          showAlert(
            "Ticket Aberto",
            `Você já possui um atendimento em ${categoria}.`
          );
          return;
        }
      } catch (e) {}
  ...
  window.startBotFlow = function (categoria) {
    ...
    setTimeout(() => {
      appendMessage(
        `Opção: <b>${categoria}</b>. Qual seu **Nome**?`,
        "received",
        true
      );
    }, 800);
  ...
  async function handleSend() {
    ...
    if (botState === "chatting") {
      ...
      if (botState === "waiting_name") {
        tempName = text;
        botState = "waiting_phone";
        setTimeout(
          () =>
            appendMessage(
              `Prazer ${tempName}. Qual seu **Telefone**?`,
              "received",
              true
            ),
          600
        ...
        try {
          ...
          saveTicketToLocal(data.session_id);
          localStorage.setItem("activeSession", data.session_id);
          statusText.innerText = data.ticket;
          setTimeout(() => {
            appendMessage(
              `Ticket ${data.ticket} criado! Alguém da equipe entrará em contato em breve.`,
              "received"
            );
            botState = "chatting";
          }, 1000);
  ...
  if (btnMic) {
    btnMic.addEventListener("click", async () => {
      if (!localStorage.getItem("activeSession"))
        return Swal.fire("Ops", "Inicie um chat primeiro.", "warning");
        ...
        try {
          ...
          mediaRecorder.onstop = async () => {
            ...
            uploadingDiv.innerHTML =
              '<i class="fas fa-circle-notch fa-spin"></i> Enviando áudio...';
    ...
    try {
      ...
      data.forEach((t) => {
        ...
        d.innerHTML = `<b>${
          ...
          t.status == "Aberto" ? "green" : "grey"
        }">${t.status}</span><br><small>${t.category}</small>`;
        d.onclick = () => {
    ...
    try {
      ...
      if (data.messages.length !== currentMessageCount) {
        ...
        data.messages.forEach((m) => {
          let c = m.conteudo;
          if (m.tipo === "audio")
            c = `<audio controls src="/static/uploads/${c}"></audio>`;
          if (m.tipo === "arquivo")
            c = `<a href="/static/uploads/${c}" target="_blank">Ver Arquivo</a>`;
